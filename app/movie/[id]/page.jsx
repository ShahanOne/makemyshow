'use client';
import ReviewCard from '../../../components/ReviewCard';
import React, { useEffect, useState } from 'react';
import { usePathname } from 'next/navigation';
import { toast } from 'react-toastify';

const Movie = () => {
  const [movieData, setMovieData] = useState({
    name: '',
    releaseDate: '',
    poster: '',
    duration: '',
    availableFor: '',
    distributer: '',
    reviews: '',
    description: '',
  });
  const [review, setReview] = useState('');
  const [stars, setStars] = useState(0);

  const pathname = usePathname();
  const movieId = pathname.split('/').pop();
  const userType = localStorage.getItem('__ut');
  const userId = localStorage.getItem('__uid');

  useEffect(() => {
    const getMovieData = async () => {
      try {
        const res = await fetch('/api/movies/get-movie', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify([
            {
              movieId: movieId,
            },
          ]),
        })
          .then((response) => response.json())
          .then((data) => {
            setMovieData(data?.movie);
          });
      } catch (err) {
        console.log(err);
      }
    };
    getMovieData();
  }, []);

  const formatDate = (dateString) => {
    const [year, month, day] = dateString.split('T')[0].split('-');
    return `${day}/${month}/${year}`;
  };

  const postReview = async () => {
    try {
      const res = await fetch('/api/movies/review', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify([
          {
            userId: userId,
            movieId: movieId,
            review: review,
            stars: stars,
          },
        ]),
      })
        .then((response) => response.json())
        .then((data) => {
          setMovieData(data?.movie);
        });
    } catch (err) {
      toast.error('Error posting review');
    }
  };

  const handleStarClick = (index) => {
    setStars(stars === index ? 0 : index);
  };
  return (
    <div className="grid grid-cols-3 gap-8 text-gray-700 p-4">
      <div className="col-span-2 rounded shadow p-4 h-screen">
        <div className="info_div pt-4">
          <p className="text-2xl">{movieData?.name}</p>
          <div className="grid grid-cols-2 py-4 gap-2">
            <p className="text-sm">
              Release Date:{' '}
              <span className="text-base">
                {formatDate(movieData?.releaseDate)}
              </span>
            </p>{' '}
            <p className="text-sm">
              Duration:{' '}
              <span className="text-base">{movieData?.duration} minutes</span>{' '}
            </p>{' '}
            <p className="text-sm">
              Distributer:{' '}
              <span className="text-base">{movieData?.distributer}</span>
            </p>{' '}
            <p className="text-sm">
              Expiring In :{' '}
              <span className="text-base">{movieData?.availableFor} Days</span>
            </p>
          </div>
          <div className="">
            <p className="text-lg">Description :</p>
            <p className="italic text-sm pt-4">
              {movieData?.description?.length > 0
                ? movieData?.description
                : 'No description provided.'}
            </p>
          </div>
        </div>
        <div className="reviews_div pt-4">
          <p className="text-lg">Reviews :</p>
          <div className="py-4 rounded-lg shadow-lg p-2 bg-indigo-100">
            <p className="italic">Watched the movie? Post a review. </p>
            <p className="py-2 text-sm">
              Stars:{' '}
              {[1, 2, 3, 4, 5].map((index) => (
                <span
                  key={index}
                  className={`cursor-pointer text-yellow-500 text-xl`}
                  onClick={() => handleStarClick(index)}
                >
                  {index <= stars ? '★' : '☆'}
                </span>
              ))}
            </p>
            <div className="flex gap-2">
              <textarea
                rows={2}
                className="w-full outline-none rounded-lg px-2 py-1 bg-gray-100"
                value={review}
                onChange={(e) => setReview(e.target.value)}
              />
              <button
                onClick={
                  userType === 'user'
                    ? postReview()
                    : toast.error('Distributers cannot post reviews!')
                }
                className="rounded-xl shadow-lg px-2 py-1 text-white bg-indigo-500 hover:bg-indigo-600"
              >
                post
              </button>
            </div>
          </div>
          <div className="flex overflow-x-scroll">
            {movieData?.reviews.length > 0 &&
              movieData?.reviews?.map((review, index) => (
                <ReviewCard
                  user={review.user}
                  stars={review.stars}
                  review={review.review}
                />
              ))}
          </div>
        </div>
      </div>
      <div className="img_div">
        <img
          className="w-full h-[30rem] shadow"
          src={movieData?.poster}
          alt="poster"
        />
      </div>
    </div>
  );
};

export default Movie;
